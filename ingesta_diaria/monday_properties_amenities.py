...
API_TOKEN = (
    "eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjE5Njk2MzQyMCwiYWFpIjoxMSwidWlkIj"
    "ozMzY5MTA2MywiaWFkIjoiMjAyMi0xMS0xOVQwOToxMjoyMS4wMDBaIiwicGVyI"
    "joibWU6d3JpdGUiLCJhY3RpZCI6MTIxMzE3ODcsInJnbiI6InVzZTEifQ."
    "ZdHFWNMZULEp188h9gSnPT8oLSmu3vHE3RMzXru4UwA"
)
API_URL = "https://api.monday.com/v2"

BOARD_ID = 4045430146
COLUMN_ID_FECHA = "pulse_log_mkqamgd4"

BIGQUERY_TABLE_ID = "properties.monday_properties_amenities"
...
def transform_items_to_rows(items):
    try:
        ...
        for item in items:
            row = {
                "pulse_id_mkqaswrw": int(item["id"]) if item["id"] else None,
...
def upsert_rows_into_bq(rows):
    ...
    schema = [
        bigquery.SchemaField("pulse_id_mkqaswrw", "INT64"),
    ...
    client.insert_rows_json(
        temp_table_id, [to_json_serializable(r) for r in rows]
    )
    print(f"{len(rows)} filas insertadas en la tabla temporal.")

    # 2️⃣ MERGE
    all_cols = [f"`{f.name}`" for f in schema]
    update_exprs = [f"T.{c} = S.{c}" for c in all_cols if c != "`pulse_id_mkqaswrw`"]
    ...
    merge_sql = f"""
        ...
        ON T.pulse_id_mkqaswrw = S.pulse_id_mkqaswrw
        WHEN MATCHED THEN
    ...
    stats = getattr(merge_job, "dml_statistics", None)
...
def main():
    ...
    while current_date <= END_DATE:
        ...
        try:
            ...
            insert_log_record(
                database_name="monday_properties_amenities",
                process_name="ingesta_diaria_monday_properties_amenities",
                records_created=0,
                records_updated=0,
                execution_time=0,
                status="ERROR",
