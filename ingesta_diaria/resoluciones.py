    ...
    try:
        ...
        for col_id, bq_type in COLUMN_TYPE_MAP.items():
            if col_id not in ("id", "name"):
                # Convertir "NUMERIC"/"FLOAT64" según corresponda
                if bq_type == "NUMERIC":
            ...
            try:
                ...
                if "Could not serialize access to table" in str(e):
                    if attempt < max_retries - 1:
                        ...
                        logging.warning(f"Conflicto de concurrencia. Reintentando en {wait_time:.2f}s (intento {attempt+1}/{max_retries})...")
                        time.sleep(wait_time)
                    else:
                        logging.error("Se alcanzó el máximo de reintentos. Abortando MERGE.")
                        raise
                else:
                    raise
...
def insert_log_record(database_name, process_name, records_created, records_updated,
    ...
    try:
        ...
        job.result()
        logging.info("Registro de log insertado correctamente en project_settings.logs.")
    except Exception as e:
...
def main():
    try:
        logging.info(f"Iniciando proceso desde {START_DATE} hasta {END_DATE} (incremento de {DELTA.days} día/s)")
        overall_inserted = 0
        overall_updated = 0
        start_time_global = time.time()
        current_date = START_DATE
        while current_date <= END_DATE:
            end_chunk = current_date  # En este caso, se procesa día a día
            if end_chunk > END_DATE:
                end_chunk = END_DATE
            logging.info(f"Consultando items creados entre {current_date} y {end_chunk} ...")
        ...
        logging.info(f"Filas totales insertadas (nuevas): {overall_inserted}, actualizadas: {overall_updated}.")
    except Exception as e:
