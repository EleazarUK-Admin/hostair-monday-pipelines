...
BOARD_ID = 4045413655
COLUMN_ID_FECHA = "pulse_log_mkqaft81"

BIGQUERY_TABLE_ID = "properties.monday_properties_accesories"
...
def transform_items_to_rows(items):
    try:
        ...
        for item in items:
            row = {}
            row["pulse_id_mkqa46c6"] = int(item["id"]) if item["id"] else None
...
def upsert_rows_into_bq(rows):
    ...
    schema = [
        bigquery.SchemaField("pulse_id_mkqa46c6", "INT64"),
    ...
    # ────── 3. Ejecutar MERGE contra la tabla destino ──────
    all_cols = [f"`{f.name}`" for f in schema]
    update_exprs = [f"T.{c} = S.{c}" for c in all_cols if c != "`pulse_id_mkqa46c6`"]
    ...
    merge_sql = f"""
        ...
        ON T.pulse_id_mkqa46c6 = S.pulse_id_mkqa46c6
        WHEN MATCHED THEN
...
def main():
    ...
    while current_date <= END_DATE:
        ...
        try:
            ...
            insert_log_record(
                database_name="monday_properties_accesories",
                process_name="ingesta_diaria_monday_properties_accesories",
                records_created=0,
                records_updated=0,
                execution_time=0,
                status="ERROR",
