...
def transform_items_to_rows(items):
    ...
    for item in items:
        row = {
            "pulse_id_mkqaj08g": int(item["id"]) if item["id"] else None,
...
def upsert_rows_into_bq(rows):
    ...
    schema = [
        bigquery.SchemaField("pulse_id_mkqaj08g", "INT64"),
    ...
    updates = [f"T.{c} = S.{c}" for c in cols if c != "`pulse_id_mkqaj08g`"]
...
def main():
    ...
    while current_date <= END_DATE:
        ...
        try:
            ...
            insert_log(
                "monday_properties_presentation_standars",
                "ingesta_diaria_monday_properties_presentation_standars",
                0, 0, 0, "ERROR",
