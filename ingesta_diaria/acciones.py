...
def fetch_items_from_monday(start_date, end_date):
    """
    Consulta a Monday los items 'CREATED_AT' entre start_date y end_date (YYYY-MM-DD).
    """
...
def transform_items_to_rows(items):
    """
    Convierte la lista de items de Monday a filas (dict) con
    columnas definidas en COLUMN_TYPE_MAP, parseando tipos.
    """
    rows = []
    for item in items:
        row = {}
...
def to_json_serializable(row):
    """
    Convierte datetime/date/time a cadenas ISO-8601 antes de hacer insert_rows_json.
    """
    new_row = {}
...
def ensure_dataset_exists(dataset_id):
    """Verifica si existe el dataset y, si no, lo crea."""
...
def upsert_rows_into_bq(rows):
    """
    Realiza un upsert (MERGE) en BigQuery:
      1) Crea una tabla temporal
      2) Inserta filas
      3) Ejecuta MERGE contra la tabla final
...
def main():
    print(f"Iniciando proceso desde {START_DATE} hasta {END_DATE} con saltos de {DELTA.days} día(s).")
    overall_inserted = 0
    overall_updated = 0
    ...
    while current_date <= END_DATE:
        # Podemos definir end_chunk para intervalos, si DELTA > 1. Aquí será día a día.
        end_chunk = current_date + DELTA - datetime.timedelta(days=1)
        if end_chunk > END_DATE:
            end_chunk = END_DATE

        # 1) Obtener items desde Monday
