...
def transform_items_to_rows(items):
    try:
        ...
        for item in items:
            row = {}
            row["id_de_elemento_mkmr9rw9"] = int(item["id"]) if item["id"] else None
...
def upsert_rows_into_bq(rows):
    ...
    schema = [bigquery.SchemaField("id_de_elemento_mkmr9rw9", "INT64"), bigquery.SchemaField("name", "STRING")]
    for col_id, bq_type in COLUMN_TYPE_MAP.items():
        if col_id not in ("id_de_elemento_mkmr9rw9", "name"):
    ...
    update_exprs = [f"T.{c} = S.{c}" for c in all_cols if c != "`id_de_elemento_mkmr9rw9`"]
...
def main():
    ...
    while current_date <= END_DATE:
        ...
        try:
            ...
            insert_log_record(
                database_name="monday_properties_general_information",
                process_name="ingesta_diaria_monday_properties_general_information",
                records_created=0,
                records_updated=0,
                execution_time=0,
                status="ERROR",
