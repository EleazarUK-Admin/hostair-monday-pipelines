...
# ID del tablero "Resoluciones"
BOARD_ID = 6908297780

# Columna de fecha (para filtrar por CREATED_AT)
COLUMN_ID_FECHA = "registro_de_creaci_n_mkkdqxf2"

# Tabla final de destino para MERGE (upsert) en BigQuery
BIGQUERY_TABLE_ID = "operations.resoluciones"

# Tabla de logs en BigQuery
LOGS_TABLE_ID = "project_settings.logs"

# Fechas y ventana de consulta
START_DATE = datetime.date.today() - datetime.timedelta(days=1)
END_DATE = datetime.date.today() - datetime.timedelta(days=1)
DELTA = datetime.timedelta(days=1)  # Procesar en chunks de varios días
...
# ------------------------------------------------------------------------------
#                  Funciones auxiliares de parseo Monday
# ------------------------------------------------------------------------------
def try_parse_json_string(s):
    ...
    try:
        ...
        print(f"[ERROR] try_parse_json_string: {e}")
        traceback.print_exc()
        return None
...
def parse_value_for_bq(value):
    ...
    try:
        ...
        # Por último, convertir a string
        return str(value)

    except Exception as e:
        print(f"[ERROR] parse_value_for_bq: {e}")
        traceback.print_exc()
        return None
...
def parse_monday_column_value(text_val, json_val, bq_type):
    """
    Convierte (text, value) de Monday a un tipo BigQuery según COLUMN_TYPE_MAP.
    """
    try:
        ...
        # Si no matchea, devolvemos string
        return str(raw_val)

    except Exception as e:
        print(f"[ERROR] parse_monday_column_value: {e}")
        traceback.print_exc()
        return None


# ------------------------------------------------------------------------------
#        Construcción del query GraphQL (filtrando por CREATED_AT)
# ------------------------------------------------------------------------------
def build_monday_query(start_str, end_str):
    ...
    try:
        ...
        print(f"[ERROR] build_monday_query: {e}")
        traceback.print_exc()
        raise
...
def fetch_items_from_monday(start_date, end_date):
    ...
    try:
        ...
        print(f"[ERROR] fetch_items_from_monday: {e}")
        traceback.print_exc()
        raise
...
def transform_items_to_rows(items):
    """
    Convierte la lista de items de Monday a filas (dict) aptas para BigQuery según COLUMN_TYPE_MAP.
    """
    try:
        rows = []
        for item in items:
        ...
        print(f"[ERROR] transform_items_to_rows: {e}")
        traceback.print_exc()
        raise
...
def to_json_serializable(row):
    ...
    try:
        ...
        print(f"[ERROR] to_json_serializable: {e}")
        traceback.print_exc()
        return row


# ------------------------------------------------------------------------------
#   Crear dataset temporal y MERGE (upsert) en tabla final de BigQuery
# ------------------------------------------------------------------------------
def ensure_dataset_exists(dataset_id):
    ...
    try:
        ...
        print(f"[ERROR] ensure_dataset_exists: {e}")
        traceback.print_exc()
        raise
...
def upsert_rows_into_bq(rows):
    """
    Realiza un MERGE (upsert) en la tabla final usando una tabla temporal 
    en el dataset 'temp_dataset'.
    ...
    try:
        ...
        for col_id, bq_type in COLUMN_TYPE_MAP.items():
            if col_id not in ("id", "name"):
                # Convertir "NUMERIC"/"FLOAT64" en algo válido
                if bq_type == "NUMERIC":
        ...
        # Inserta filas en la tabla temporal
            ...
            try:
                ...
                if "Could not serialize access to table" in str(e):
                    if attempt < max_retries - 1:
                        ...
                        print(f"[WARN] Conflicto de concurrencia. Reintentando en {wait_time:.2f}s (intento {attempt+1}/{max_retries})...")
        ...
        print(f"[ERROR] upsert_rows_into_bq: {e}")
        traceback.print_exc()
        raise


# ------------------------------------------------------------------------------
#          Insertar LOG al terminar la ingesta en BigQuery (opcional)
# ------------------------------------------------------------------------------
def insert_log_record(database_name, process_name, records_created, records_updated,
    ...
    try:
        ...
        print(f"[ERROR] insert_log_record: {e}")
        traceback.print_exc()
...
def main():
    try:
        print(f"Iniciando proceso desde {START_DATE} hasta {END_DATE} (delta={DELTA.days} día/s)")
        overall_inserted = 0
        overall_updated = 0
        ...
        insert_log_record(
            ...
            process_name="ingesta_diaria_resoluciones",
        ...
        print(f"[ERROR] Error en la ejecución del proceso principal: {e}")
        traceback.print_exc()

    finally:
        print("Finalizando el script. No se utiliza Cloud Logging, solo prints.")
