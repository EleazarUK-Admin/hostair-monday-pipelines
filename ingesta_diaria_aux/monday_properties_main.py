    ...
    try:
        ...
        if json_val:
            try:
                ...
                pass
        parsed_from_text = parse_value_for_bq(text_val) if not parsed_from_json_val else None
...
def fetch_items_from_monday():
    try:
        ...
        items_list = boards_data[0].get("items_page", {}).get("items", []) if boards_data else []
        return items_list
    except Exception as e:
...
def transform_items_to_rows(items):
    try:
        ...
        for item in items:
            row = {}
            row["id_de_elemento_mkn314vp"] = int(item["id"]) if item["id"] else None
...
def upsert_rows_into_bq(rows):
    ...
    schema = [bigquery.SchemaField("id_de_elemento_mkn314vp", "INT64"), bigquery.SchemaField("name", "STRING")]
    for col_id, bq_type in COLUMN_TYPE_MAP.items():
        if col_id not in ("id_de_elemento_mkn314vp", "name"):
    ...
    update_exprs = [f"T.{c} = S.{c}" for c in all_cols if c != "`id_de_elemento_mkn314vp`"]
...
def main():
    ...
    while current_date <= END_DATE:
        ...
        try:
            ...
            insert_log_record(
                database_name="monday_properties_main",
                process_name="ingesta_diaria_monday_properties_main",
                records_created=0,
                records_updated=0,
                execution_time=0,
                status="ERROR",
