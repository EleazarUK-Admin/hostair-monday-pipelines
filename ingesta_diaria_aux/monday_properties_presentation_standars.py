...
BOARD_ID = 4045514460
COLUMN_ID_FECHA = "pulse_log_mkqa9355"

BIGQUERY_TABLE_ID = "properties.monday_properties_presentation_standars"
...
COLUMN_TYPE_MAP = {
    ...
    "archivo7": "STRING",
    "archivo47": "STRING",
    "archivo_mkn5as9y": "STRING",
    "archivo_mkn5dvpr": "STRING",
    "status": "STRING",
    "link_to_propiedades": "STRING",
    "pulse_log_mkqa9355": "TIMESTAMP",
    "pulse_updated_mkqa8r0v": "TIMESTAMP",
}
...
def transform_items_to_rows(items):
    try:
        ...
        for item in items:
            row = {}
            row["pulse_id_mkqaj08g"] = int(item["id"]) if item["id"] else None
...
def upsert_rows_into_bq(rows):
    ...
    schema = [bigquery.SchemaField("pulse_id_mkqaj08g", "INT64"), bigquery.SchemaField("name", "STRING")]
    ...
    update_exprs = [f"T.{c} = S.{c}" for c in all_cols if c != "`pulse_id_mkqaj08g`"]
    ...
    merge_sql = f"""
        ...
        ON T.pulse_id_mkqaj08g = S.pulse_id_mkqaj08g
...
def main():
    ...
    while current_date <= END_DATE:
        ...
        try:
            ...
            insert_log_record(
                database_name="monday_properties_presentation_standars",
                process_name="ingesta_diaria_monday_properties_presentation_standars",
                records_created=0,
                records_updated=0,
                execution_time=0,
                status="ERROR",
