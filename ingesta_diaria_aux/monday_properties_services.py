...
def transform_items_to_rows(items):
    try:
        ...
        for item in items:
            row = {}
            row["pulse_id_mkqaxmpc"] = int(item["id"]) if item["id"] else None
...
def upsert_rows_into_bq(rows):
    ...
    schema = [bigquery.SchemaField("pulse_id_mkqaxmpc", "INT64"), bigquery.SchemaField("name", "STRING")]
    ...
    update_exprs = [f"T.{c} = S.{c}" for c in all_cols if c != "`pulse_id_mkqaxmpc`"]
...
def main():
    ...
    while current_date <= END_DATE:
        ...
        try:
            ...
            insert_log_record(
                database_name="monday_properties_services",
                process_name="ingesta_diaria_monday_properties_services",
                records_created=0,
                records_updated=0,
                execution_time=0,
                status="ERROR",
