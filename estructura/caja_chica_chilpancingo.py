...
BOARD_ID = 7269476761           # ID de tu board en Monday
COLUMN_ID_FECHA = "date"        # ID de la columna tipo DATE en Monday
BIGQUERY_TABLE_ID = "finance.caja_chica_chilpancingo"

# Rango de fechas que deseas cubrir
START_DATE = datetime.date(2024, 1, 1)
END_DATE   = datetime.date(2026, 1, 1)

# Incremento de 2 días por iteración
DELTA = datetime.timedelta(days=2)

# Mapeo de columnas en Monday -> tipo en BigQuery
COLUMN_TYPE_MAP = {
    "id": "INT64",
    "name": "STRING",
...
def parse_value_for_bq(value):
    ...
    if isinstance(value, str):
        ...
        # No es JSON ni numérico => se devuelve tal cual
        return value

    # Si es lista u otro => string
    return str(value)
...
def parse_monday_column_value(text_val, json_val, bq_type):
    ...
    y luego convertirlo al tipo de BigQuery correspondiente.
    """
...
def build_monday_query(start_str, end_str):
    """
    Construye la consulta GraphQL para filtrar items por la columna "date".
    """
    query = f"""
    query {{
      boards(ids: {BOARD_ID}) {{
...
def fetch_items_from_monday(start_date, end_date):
    """Consulta la API de Monday para items cuya fecha 'date' esté entre start_date y end_date."""
...
def insert_rows_into_bq(rows):
    """Inserta las filas en la tabla finance.caja_chica_chilpancingo."""
    if not rows:
        print("No hay filas para insertar en este rango.")
        return
