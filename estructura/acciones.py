...
BOARD_ID = 5355817123  # Board de "acciones"
COLUMN_ID_FECHA = "registro_de_creaci_n__1"  # Filtraremos por esta columna (TIMESTAMP en Monday)
...
# Mapeo de las columnas con su tipo en BigQuery
COLUMN_TYPE_MAP = {
    "id": "NUMERIC",
    "name": "STRING",
    "subelementos": "STRING",
...
def build_monday_query(start_str, end_str):
    """
    Ignoramos end_str, y usamos operator: any_of, EXACT, compare_attribute: CREATED_AT 
    para cada día.
    """
    query = f"""
    query {{
...
def fetch_items_from_monday(start_date, end_date):
    """
    Solo filtramos por la fecha EXACT de 'start_date', 
    porque la API no permite un rango en items_page con este método.
    """
...
def insert_rows_into_bq(rows):
    """
    Inserta (append) las filas en la tabla operations.acciones.
    """
    if not rows:
...
def main():
    print(f"Procesando datos desde {START_DATE} hasta {END_DATE}, día a día...\n")
    ...
    while current_date <= END_DATE:
        ...
        # Debido a DELTA=1, 'current_date' == 'end_chunk' en cada paso
        print(f"Obteniendo items del {current_date} (EXACT CREATED_AT) ...")
