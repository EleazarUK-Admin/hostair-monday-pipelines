...
BOARD_ID = 2663242816  # ID del board de Solicitudes de Mantenimiento
COLUMN_ID_FECHA = "creaci_n_de_registro"  # (TIMESTAMP en Monday)
BIGQUERY_TABLE_ID = "maintenance.solicitudes_mantenimiento"

# Ajusta el rango de fechas si lo requieres:
START_DATE = datetime.date(2020, 1, 1)
END_DATE   = datetime.date(2027, 1, 1)

# Ahora lo haremos día a día.
DELTA = datetime.timedelta(days=1)

# Mapeo de las columnas según la estructura de la tabla
COLUMN_TYPE_MAP = {
    "id": "NUMERIC",
    "name": "STRING",
...
def build_monday_query(start_str, end_str):
    """
    CAMBIO: Ahora ignoramos end_str y usamos operator: any_of con EXACT 
    para la fecha 'start_str' en compare_attribute: CREATED_AT.
    """
    query = f"""
    query {{
      boards(ids: {BOARD_ID}) {{
        items_page(
          query_params: {{
            rules: [{{
              ...
              compare_value: ["{start_str}", "{start_str}"],
              operator: between,
...
def fetch_items_from_monday(start_date, end_date):
    """
    Llama a la API de Monday, pero NOTA: ahora 'end_date' se ignora
    en el query, porque solo podemos hacer 'EXACT' con 'start_date'.
    """
    # Con DELTA = 1 día, start_date == end_date en cada iteración,
    # y lo usaremos como la fecha EXACT a filtrar.
    day_str = start_date.strftime("%Y-%m-%d")
    # end_str no se ocupa, pero lo dejamos para respetar la firma.
    # end_str = end_date.strftime("%Y-%m-%d")  # Se ignora en el query
...
def insert_rows_into_bq(rows):
    """
    Inserta (append) las filas en la tabla maintenance.solicitudes_mantenimiento.
...
def main():
    print(f"Procesando datos desde {START_DATE} hasta {END_DATE}, en intervalos de {DELTA.days} día(s)...")
    ...
    while current_date <= END_DATE:
        ...
        # Debido a que DELTA=1 día, "current_date" == "end_chunk" en cada paso
        print(f"\nObteniendo items del {current_date} (EXACT CREATED_AT) ...")
