...
def transform_items_to_rows(items):
    try:
        ...
        for item in items:
            row = {
                "pulse_id_mkqak00g": int(item["id"]) if item["id"] else None,
...
def upsert_rows_into_bq(rows):
    ...
    schema = [
        bigquery.SchemaField("pulse_id_mkqak00g", "INT64"),
        bigquery.SchemaField("name", "STRING"),
    ] + [
        bigquery.SchemaField(cid, tp) for cid, tp in COLUMN_TYPE_MAP.items()
        if cid not in ("pulse_id_mkqak00g", "name")
    ]
    ...
    updates = [f"T.{c} = S.{c}" for c in cols if c != "`pulse_id_mkqak00g`"]
...
# ───────── Logging ─────────
def insert_log(database, process, created, updated, secs, status, first, last):
    try:
        bigquery.Client().query(
            ...
            VALUES (
              CURRENT_TIMESTAMP(), '{database}', '{process}',
...
def main():
    ...
    while current_date <= END_DATE:
        ...
        try:
            ...
            insert_log(
                "monday_properties_equipment",
                "ingesta_diaria_monday_properties_equipment",
                0, 0, 0, "ERROR",
