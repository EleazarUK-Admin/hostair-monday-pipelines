...
def fetch_items_from_monday(date_obj):
    ...
    query = build_monday_query(day_str)
...
def get_bq_parameter(key, value):
    ...
    if t == "NUMERIC":
        return bigquery.ScalarQueryParameter(f"param_{key}", "NUMERIC", float(value) if value is not None else None)
    if t == "DATE":
...
def merge_rows_into_bq(rows):
    ...
    for row in rows:
        ...
        id_param = bigquery.ScalarQueryParameter("id", "NUMERIC", row_serializable["id"])
        ...
        if results and results[0].count > 0:
            ...
            parameters.append(bigquery.ScalarQueryParameter("id", "NUMERIC", row_serializable["id"]))
