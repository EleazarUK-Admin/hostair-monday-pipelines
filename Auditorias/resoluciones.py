...
# Columna de fecha para filtrar en Monday
COLUMN_ID_FECHA = "registro_de_creaci_n_mkkdqxf2"

# Tabla final de destino en BigQuery
BIGQUERY_TABLE_ID = "operations.resoluciones"

# Tablas de logs y auditoría (se asume que la tabla de auditoría tiene el siguiente schema):
...
def parse_value_for_bq(value):
    ...
    primitivo (str, num, bool, etc.) para mapearlo a BigQuery.
    """
    if value is None:
        return None
...
def parse_monday_column_value(text_val, json_val, bq_type):
    """
    Dado el text y value de una columna en Monday, lo convierte al tipo BigQuery deseado.
    """
    parsed_from_json_val = None
    if json_val:
        try:
            as_dict = json.loads(json_val)
            # Evitamos usar estructuras con "index"
...
def build_monday_query(start_str, end_str):
    """
    Construye un query GraphQL para filtrar items según la columna 'date'
...
def transform_items_to_rows(items):
    """
    Convierte los items de Monday a un arreglo de dicts, mapeando cada columna
    al tipo definido en COLUMN_TYPE_MAP.
    """
    rows = []
    for item in items:
        row = {}
...
def to_json_serializable(row):
    """
    Convierte objetos datetime/date/time a cadenas ISO-8601 para insert_rows_json.
    """
    new_row = {}
...
def audit_rows_in_bq(rows, start_date, end_date, monday_query):
    ...
    y registra en la tabla de auditoría:
      - Cantidad de registros según Monday y en BigQuery.
      - El ID más antiguo y el más nuevo en ambos orígenes.
      - La diferencia y un mensaje detallado.
    El query de Monday (monday_query) se usa solo para referencia en auditoría (no se inserta).
    """
    client = bigquery.Client()
    ...
    # Extraer los IDs de los registros de Monday (se asume que 'id' ya es numérico)
...
def main():
    ...
    # Procesar día a día (o en intervalos definidos en DELTA)
    while current_date <= END_DATE:
        end_chunk = current_date  # En este caso, se procesa por día
    ...
    insert_log_record(
        database_name="resoluciones",
        process_name="auditoria_diaria_resoluciones",
