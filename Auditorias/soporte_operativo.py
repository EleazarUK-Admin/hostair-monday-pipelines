...
# Board y columna de fecha en Monday
BOARD_ID = 5914627798
COLUMN_ID_FECHA = "creaci_n_de_registro"  # Se usará compare_attribute: CREATED_AT

# Tabla destino en BigQuery
BIGQUERY_TABLE_ID = "support.soporte_operativo"

# Tablas de logs y auditoría (auditoría se insertará en project_settings.auditoria)
...
# Nota: Se ha corregido "INT 64" por "INT64"
COLUMN_TYPE_MAP = {
    "id": "INT64",
    "name": "STRING",
...
def build_monday_query(start_str, end_str):
    """
    Genera un query GraphQL para filtrar por la columna 'creaci_n_de_registro'
    entre start_str y end_str (YYYY-MM-DD), usando compare_attribute='CREATED_AT'.
    """
    query = f"""
    query {{
      boards(ids: {BOARD_ID}) {{
...
def fetch_items_from_monday(start_date, end_date):
    ...
    (usando la columna 'creaci_n_de_registro' y compare_attribute='CREATED_AT').
    """
...
def transform_items_to_rows(items):
    """
    Convierte la lista de items de Monday a filas aptas para BigQuery,
    mapeando cada columna según COLUMN_TYPE_MAP.
    """
    rows = []
    for item in items:
        row = {}
...
def audit_rows_in_bq(rows, start_date, end_date, monday_query):
    ...
    El query de Monday se utiliza solo para referencia.
    """
    client = bigquery.Client()
    ...
    # Determinar el ID más antiguo y el más nuevo según Monday
...
def main():
    ...
    # Procesar en intervalos (día a día en este caso)
    while current_date <= END_DATE:
        end_chunk = current_date  # Procesamos un día a la vez
    ...
    insert_log_record(
        database_name="support",
        process_name="auditoria_diaria_soporte_operativo",
