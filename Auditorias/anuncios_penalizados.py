...
# El ID del board para "anuncios_penalizados"
BOARD_ID = 7019548313
...
# Tabla de logs y tabla de auditoría (se asume que el schema de auditoría es el siguiente):
...
def parse_monday_column_value(text_val, json_val, bq_type):
    """
    Dado el text y value de una columna en Monday, parsearlo al tipo BigQuery deseado.
    """
...
def build_monday_query(start_str, end_str):
    ...
    en la columna 'date' entre start_str y end_str (formato YYYY-MM-DD).
    """
    query = f"""
    query {{
      boards(ids: {BOARD_ID}) {{
...
def main():
    ...
    insert_log_record(
        database_name="anuncios_penalizados",
        process_name="auditoria_diaria_anuncios_penalizados",
